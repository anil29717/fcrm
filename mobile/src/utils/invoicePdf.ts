...
> = {
  ...
  signature: (ctx) => {
    ...
    return `<div class="block signature"><div class="sig-line"></div><div>Authorized Signatory</div><div>${esc(name)}</div></div>`;
  },
};
...
async function buildInvoiceHtml(invoiceId: number): Promise<string> {
  ...
  const ctx: InvoicePdfContext = { company, invoice, lineItems, logoDataUri, blocks };
...
<head>
  ...
  <style>
    ...
    .sig-line { border-top: 1px solid #333; width: 200px; margin-bottom: 6px; }
  </style>
</head>
<body>
  ${blockHtml}
